...
for (const scenario of ['image-only', 'upload-failure', 'download-failure', 'html-response', 'empty-image', 'no-image']) {
  test(`busker refresh: ${scenario}`, async (t) => {
    ...
    if (['upload-failure', 'download-failure', 'html-response', 'empty-image'].includes(scenario)) {
    ...
    assert.equal(imageDownloads, scenario === 'no-image' ? 0 : 1);
    assert.equal(uploads.length, ['image-only', 'upload-failure'].includes(scenario) ? 1 : 0);
